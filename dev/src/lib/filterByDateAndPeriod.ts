...
export function filterByDateAndPeriod(period: "afternoon" | "evening" | "morning", chosenDateTime: Date, availableSlotsForDate: string[]) {
  ...
  const filtered = availableSlotsForDate.filter((availability) => {
    ...
    const range = availabilityMoment.isBetween(currentRange.start, currentRange.end, "minute");
    return range;
  });
