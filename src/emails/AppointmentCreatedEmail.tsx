...
export const Email = ({ doc }: Props) => (
  <Html>
    <Head />
    <Preview>Your appointment has been booked!</Preview>
    <Tailwind>
      <Body>
        <Container className="outline-neutral-300 bg-white outline-[1px] outline">
          ...
          <Section className="p-6">
            ...
            <Heading className="text-[18px]">Your appointment with {doc?.host.firstName} has been booked</Heading>
            <Container className="bg-neutral-200  p-6 rounded-sm my-6">
              ...
              <Text>
                Date: <span className="font-bold">{moment(doc?.start).format("MMMM Do YYYY")}</span>
              </Text>
              <Text>
                Service: <span className="font-bold">{doc?.services.map((service) => service.title).join(", ")}</span>
              </Text>
              <Text className="m-0">
            ...
            <Button href="https://linear.app">Reschedule</Button> <Button href="https://linear.app">Cancel</Button>
            <Hr className="border-neutral-300" />
            <Text className="mb-0">
              Regard,
              <br />
          ...
          <Section className="p-6 bg-neutral-100 text-neutral-500 my-0 text-center">
            <Text className="m-0 mb-2">
              This email was intended for {doc?.customer.firstName} {doc?.customer.lastName}. If you are not this person contact us immediately.
            </Text>
            <Text className="m-0">Please contact us if you have any questions. (If you reply to this email, we won't be able to see it.)</Text>
          </Section>
        </Container>
      </Body>
    </Tailwind>
