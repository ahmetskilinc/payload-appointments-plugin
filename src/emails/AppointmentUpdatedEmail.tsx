      ...
      <Body>
        <Container className="outline-neutral-300 bg-white outline-[1px] outline">
          ...
          <Section className="p-6">
            ...
            <Heading className="text-[18px]">Your appointment with {doc?.host.firstName} has been updated</Heading>
            <Container className="bg-neutral-200  p-6 rounded-sm my-6">
              ...
              <Text>
                New date: <span className="font-bold">{moment(doc?.start).format("MMMM Do YYYY")}</span>
              </Text>
              <Text>
                Service{doc?.services.length! > 1}: <span className="font-bold">{doc?.services.map((service) => service.title).join(", ")}</span>
              </Text>
              <Text className="m-0">
